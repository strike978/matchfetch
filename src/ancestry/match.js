(function () {
  ...
  function fetchCurrentProfileData() {
    return apiFetch('https://www.ancestry.com/discoveryui-matches/cluster/api/profileData/' + guid, {
      method: 'POST', credentials: 'include', mode: 'cors',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      body: JSON.stringify({ matchSampleIds: [sampleId] })
    }).then(function (data) {
      if (!data || typeof data !== 'object') return
      var profile = data[sampleId] || {}
      var p = s.matchData && s.matchData.profile
      if (p) {
        p.matchName = profile.matchName || null
        p.matchNameInitials = profile.matchNameInitials || null
        p.displayGender = profile.displayGender || null
        p.photoUrl = profile.photoUrl || null
      }
      if (typeof DB !== 'undefined' && DB.setMatchProfile) DB.setMatchProfile(guid, sampleId, profile)
      setState({ matchData: s.matchData })
    }).catch(function () { })
  }
        ...
        tags.length > 0
          ? m('.group-manager-list', tags.map(function (t) {
              ...
              return m('.group-manager-row', { key: t.tagId }, [
                ...
                m('button.group-btn', { onclick: function () { s.editingTagId = t.tagId; s.renameName = t.label; m.redraw() } }, 'Edit'),
                m('button.group-btn.group-btn-danger', { onclick: function () {
                  s.modal = {
                    title: 'Delete group?',
                    icon: '<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',
