...
- **Fetch Options** — AncestryDNA: fetch all matches, limit by count, or by cM range. 23andMe: fetch all matches, limit by count, cM range, or grandparent birth country. Both can resume an incomplete fetch.
- **Check for New Matches** — After fetching, pick up any new matches since the last fetch.
...
4. **23andMe** — select a profile, then click **Fetch** (or expand **Fetch options** for Count / cM Range / Country) to fetch matches and pull in their ancestry and haplogroups.
...
Created by Omar Nunez · Version 1.4.0
